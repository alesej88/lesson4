public class Main {
    private static Object monitor = new Object();
    private static final int repetitionQuantity = 5;
    private static volatile char lastLetter = 'C';


    public static void main(String[] args) {
        LetterPrinterThread threadA = new LetterPrinterThread('C','A');
        LetterPrinterThread threadB = new LetterPrinterThread('A','C');
        LetterPrinterThread threadC = new LetterPrinterThread('A','B');

        threadA.start();
        threadB.start();
        threadC.start();
    }
    private static class LetterPrinterThread extends Thread{
        private char before;
        private char after;

        public LetterPrinterThread(char before, char after){
            this.before = before;
            this.after = after;
        }
        @Override
        public void run(){
            try {
                for (int i = 0; i < repetitionQuantity; i++) {
                    synchronized (monitor) {
                        while (lastLetter != before) ;
                        monitor.wait();
                    }
                    System.out.println(after);
                    lastLetter = after;
                    monitor.notifyAll();

                }

            }catch (InterruptedException e){
                e.printStackTrace();
            }

        }
    }
}
